...
server <- function(input, output) {
  dataInput <- reactive({
    ...
    if (station == "Lalashan"){
      read_data <- read.csv(paste0("data/Lalashan2008-2017dia.csv"), stringsAsFactors = F)
      rownames(read_data) <- c("Prec.", "Mean.t", "Max.t", "Min.t", "Ab.max.t", "Ab.min.t")
      colnames(read_data) <- as.character(1:12)
      return(read_data)
    } else {
      years <- input$years
  ...
  output$diagram <- renderPlot({
    if (input$station == "Lalashan"){
      diagwl(dataInput()[c("Prec.", "Max.t", "Min.t", "Ab.min.t"),], est = input$station, alt = station_list[2, input$station], per = paste(2008, 2017, sep = "-"), mlab = "en")
    } else {
...
# shiny::runApp("D:/6111/VegLab/R & Statistics/climate_diagram", display.mode = "showcase")
# shiny::runApp("D:/6111/VegLab/R & Statistics/climate_diagram")
